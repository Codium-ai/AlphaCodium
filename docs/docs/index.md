...
For other models like DeepSeek, we cannot be sure. However, note that our [main result](https://github.com/Codium-ai/AlphaCodium/blob/main/pics/comparison.png?raw=true) is a comparison of "direct prompt" vs. "qodo Flow". Data leakage would help both approaches, so the relative improvement of qodo Flow flow is still valid.
___
...
**A:** In comparison to AlphaCode, we do four orders of magnitude (!) fewer [calls](https://github.com/Codium-ai/AlphaCodium/blob/main/pics/computational_effort.png?raw=true) (per solution qodo Flow does 15-20 calls).
...
First and foremost, we feel that the proposed qodo Flow [flow](https://github.com/Codium-ai/AlphaCodium/blob/main/pics/proposed_flow.png?raw=true), with reasonable adjustments, can be used as a more general framework for other code generation tasks.
...
```
@misc{ridnik2024code,
      title={Code Generation with qodo Flow: From Prompt Engineering to Flow Engineering}, 
