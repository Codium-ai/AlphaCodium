...
async def run_choose_best_solution(self, problem):
    ...
    while True:
        try:
            ...
            #TODO if only one possible solution, no inference needed
            ...
            response_best_solution_yaml = load_yaml(response_best_solution,
                                                    keys_fix_yaml=["name:", "content:", "why:", "flow:", "- "])

            problem['s_best_solution'] = response_best_solution_yaml
            problem['s_best_solution_str'] = response_best_solution.strip()

            # set best solution, and other solutions
            problem['s_other_solutions'] = []

            if 's_possible_solutions' in problem:
                problem['s_other_solutions'] = []
                for solution in problem['s_possible_solutions']:
                    if solution['name'].strip().lower() != response_best_solution_yaml['name'].strip().lower():
